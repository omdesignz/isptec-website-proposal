...
<div class="px-4 sm:px-6 lg:px-8 mt-8">
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        <h1 class="text-xl font-semibold text-gray-900">{{ $setting['option'] }}</h1>
    ...
    <form method="post" action="{{ route('settings.settranslation', ['setting' => $setting['id'], 'lang' => $lang]) }}" class="space-y-8 divide-y divide-gray-200" enctype="multipart/form-data">
        @method('PUT')  
        @csrf  
        ...
        <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
          <dl class="sm:divide-y sm:divide-gray-200">
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Parâmetro</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $setting['option'] }}" type="text" name="option" id="option" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('option')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('option'))
                    <p class="mt-2 text-sm text-red-500">{{ $errors->first('option') }}</p>
                    @endif
              </dd>
            </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                  <textarea id="value" name="value" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('value')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">{{ $setting['value'] }}</textarea>
                    </div>
                    @if($errors->has('value'))
                    <p class="mt-2 text-sm text-red-500">{{ $errors->first('value') }}</p>
                    @endif
              </dd>
            </div>
