...
@section('search_url')
{{ route('recruitmentcategories.index', []) }}
@endsection
...
<div class="px-4 sm:px-6 lg:px-8 mt-8">
          ...
          <tr x-data="{openLangDropdown_{{ $category->id }}: false}">
            ...
            <td class="relative py-2.5 pl-3 pr-4 sm:pr-6 text-right text-sm font-medium">
                <span class="relative z-0 inline-flex shadow-sm rounded-md">
                    <a href="{{ route('recruitmentcategories.show', ['category' => $category['id']]) }}" class="relative inline-flex items-center px-2.5 py-1.5 text-xs font-medium shadow-sm rounded-l-md text-white bg-gray-900 hover:bg-yellow-400">
                    ...
                    {{-- <a href="{{ route('recruitmentcategories.edit', ['category' => $category->id]) }}" class="-ml-px relative inline-flex items-center px-2.5 py-1.5 text-xs font-medium shadow-sm text-white bg-gray-900 hover:bg-yellow-400">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transform transition-all duration-200 hover:scale-150 hover:text-gray-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    @if ($category->deleted_at)
                      <a href="{{ route('recruitmentcategories.restore', ['category' => $category->id]) }}" class="-ml-px relative inline-flex items-center px-2.5 py-1.5 text-xs font-medium shadow-sm rounded-r-md text-white bg-gray-900 hover:bg-yellow-400">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transform transition-all duration-200 hover:scale-150 hover:text-gray-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    @endif
                    @if (!$category->deleted_at)
                      <a href="{{ route('recruitmentcategories.destroy', ['category' => $category->id]) }}" class="-ml-px relative inline-flex items-center px-2.5 py-1.5 text-xs font-medium shadow-sm rounded-r-md text-white bg-gray-900 hover:bg-yellow-400">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transform transition-all duration-200 hover:scale-150 hover:text-gray-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  ...
                  <div class="z-10 origin-top-right absolute right-0 mt-2 w-24 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1" x-cloak x-show="openLangDropdown_{{ $category->id }}" @click.away="openLangDropdown_{{ $category->id }} = false">
                    <div class="py-1" role="none">
                      <a href="{{ route('recruitmentcategories.getlang', ['category' => $category->id, 'lang' => 'en']) }}" class="text-gray-700 hover:text-gray-800 flex items-center m-2">
                      ...
                      <a href="{{ route('recruitmentcategories.getlang', ['category' => $category->id, 'lang' => 'pt']) }}" class="text-gray-700 hover:text-gray-800 flex items-center m-2">
  ...
  @include('admin.recruitmentcategory._slideover')
  {{-- @include('admin.recruitmentcategories._lang_modal') --}}
</div>
