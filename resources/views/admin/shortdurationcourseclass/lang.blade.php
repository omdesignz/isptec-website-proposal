...
@section('title')
Cursos de Curta Duração - Turmas
@endsection

@section('search_helper')
Insira o nome da turma e prima ENTER para pesquisar...
@endsection

@section('search_url')
{{ route('shortdurationcourseclasses.index', []) }}
@endsection
...
<div class="px-4 sm:px-6 lg:px-8 mt-8">
    ...
    <form method="post" action="{{ route('shortdurationcourseclasses.settranslation', ['class' => $class['id'], 'lang' => $lang]) }}" class="space-y-8 divide-y divide-gray-200" enctype="multipart/form-data" x-data="imgPreview" x-cloak>
        @method('PUT')  
        @csrf  
        ...
        <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
          <dl class="sm:divide-y sm:divide-gray-200">
                ...
                <div class="mt-1 sm:col-span-2 sm:mt-0">
                  <select id="course_id" name="course_id" class="block w-full max-w-lg rounded-md border-gray-300 shadow-sm sm:max-w-xs sm:text-sm">
                    @foreach ($courses as $course)
                        <option value="{{ $course['id'] }}" @selected($class['course_id'] == $course['id'])>{{ $course['name'] }}</option>
                    @endforeach
                  </select>
                </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $class['name'] }}" type="text" name="name" id="name" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('name')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('name'))
          ...
          <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Horário: Entrada</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $class['start_time'] }}"
                           type="time"
                           step="2"
                           name="start_time" 
                           id="start_time" 
                           class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('start_time')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('start_time'))
                    <p class="mt-2 text-sm text-red-500">{{ $errors->first('start_time') }}</p>
                    @endif
              </dd>
          </div>

          <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt class="text-sm font-medium text-gray-500">Horário: Saída</dt>
            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
              <div class="mt-1">
                  <input value="{{ $class['end_time'] }}"
                        type="time"
                        step="2"
                        name="end_time" 
                        id="end_time" 
                        class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('end_time')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                  </div>
                  @if($errors->has('end_time'))
                  <p class="mt-2 text-sm text-red-500">{{ $errors->first('end_time') }}</p>
                  @endif
            </dd>
        </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $class['total_hours'] }}" type="number" name="total_hours" id="total_hours" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('total_hours')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('total_hours'))
                    <p class="mt-2 text-sm text-red-500">{{ $errors->first('total_hours') }}</p>
                    @endif
              </dd>
            </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $class['price'] }}" type="number" name="price" id="price" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('price')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('price'))
                    <p class="mt-2 text-sm text-red-500">{{ $errors->first('price') }}</p>
                    @endif
              </dd>
            </div>

            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Taxa de Inscrição</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $class['registration_fee'] }}" type="number" name="registration_fee" id="registration_fee" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('registration_fee')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('registration_fee'))
                    <p class="mt-2 text-sm text-red-500">{{ $errors->first('registration_fee') }}</p>
                    @endif
              </dd>
            </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                  <textarea id="description" name="description" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('description')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">{{ $class['description'] }}</textarea>
                    </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                  <textarea id="extra_data" name="extra_data" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('extra_data')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">{{ $class['extra_data'] }}</textarea>
                    </div>
                    ...
                    <div class="divide-y divide-gray-200">
                      <div class="relative flex items-start py-4">
                        <div class="min-w-0 flex-1 text-sm">
                          ...
                          <p id="status-description" class="text-gray-500">Define se a turma em questão continua activa.</p>
                        </div>
                        <div class="ml-3 flex h-5 items-center">
                          <input value="1" @checked($class['status']) id="status" aria-describedby="status-description" name="status" type="checkbox" class="h-4 w-4 rounded-full border-gray-300 text-yellow-400">
                        </div>
                      </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                @if (count($class['featured_image']) > 0)
                  <img src="{{ $class['featured_image'][0]?->getUrl() }}" alt="" class="object-cover h-96 w-96 rounded-2xl" loading="lazy" />
                @endif
                  ...
                  <div>
                    <div class="flow-root mt-6">
                      <ul role="list" class="-my-5 divide-y divide-gray-200">
                        @forelse ($class['documents'] as $document)
                          <li class="py-4">
                            <div class="flex items-center space-x-4">
                              ...
                              <div>
                                <a href="{{ route('shortdurationcourseclasses.deletesingleattachment', ['model_id' => $document->id]) }}" class="ml-3 rounded-md inline-flex justify-center px-2.5 py-1.5 text-xs font-medium bg-gray-900 text-white shadow-sm hover:bg-white hover:text-red-500 sm:w-auto">
