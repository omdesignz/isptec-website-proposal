    ...
    <div class="fixed inset-0 overflow-hidden">
      <div class="absolute inset-0 overflow-hidden">
        <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10 sm:pl-16">
          ...
          >
            <form class="flex h-full flex-col divide-y divide-gray-200 bg-white shadow-xl" action="/admin/semesters" method="POST">
              @csrf
              <div class="h-0 flex-1 overflow-y-auto">
                <div class="bg-yellow-400 py-6 px-4 sm:px-6">
                  <div class="flex items-center justify-between">
                    <h2 class="text-lg font-medium text-gray-900" id="slide-over-title">Novo Semestre</h2>
                ...
                <div class="flex flex-1 flex-col justify-between">
                  <div class="divide-y divide-gray-200 px-4 sm:px-6">
                    <div class="space-y-6 pt-6 pb-5">
                      ...
                      <div>
                        <label for="year" class="block text-sm font-medium text-gray-900"> Ano Lectivo </label>
                        <div class="mt-1">
                          <textarea id="year" name="year" rows="4" class="block w-full rounded-md border border-gray-300 shadow-sm focus:border-rose-500 focus:ring-rose-500 sm:text-sm"></textarea>
                        </div>
                        @if($errors->has('year'))
                      ...
                      <div>
                        ...
                        <fieldset class="border-t border-b border-gray-200">
                          <legend class="sr-only">Activo?</legend>
                          <div class="divide-y divide-gray-200">
                            <div class="relative flex items-start py-4">
                              <div class="min-w-0 flex-1 text-sm">
                                <label for="status" class="font-medium text-gray-700">Activo?</label>
                                <p id="status-description" class="text-gray-500">Define se o semestre em questão continua activo.</p>
                              </div>
                              <div class="ml-3 flex h-5 items-center">
                                <input value="1" id="status" aria-describedby="status-description" name="status" type="checkbox" class="h-4 w-4 rounded-full border-gray-300 text-yellow-400">
                              </div>
                            </div>
