...
<div class="px-4 sm:px-6 lg:px-8 mt-8">
    ...
    <form method="post" action="{{ route('semesters.settranslation', ['semester' => $semester['id'], 'lang' => $lang]) }}" class="space-y-8 divide-y divide-gray-200" enctype="multipart/form-data">
        @method('PUT')  
        @csrf  
          ...
          <dl class="sm:divide-y sm:divide-gray-200">
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $semester['name'] }}" type="text" name="name" id="name" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('name')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('name'))
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                  <textarea id="year" name="year" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('year')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">{{ $semester['year'] }}</textarea>
                    </div>
                    @if($errors->has('year'))
                    ...
                    <div class="divide-y divide-gray-200">
                      <div class="relative flex items-start py-4">
                        <div class="min-w-0 flex-1 text-sm">
                          ...
                          <p id="status-description" class="text-gray-500">Define se o semestre em questão continua activo.</p>
                        </div>
                        <div class="ml-3 flex h-5 items-center">
                          <input value="1" @checked($semester['status']) id="status" aria-describedby="status-description" name="status" type="checkbox" class="h-4 w-4 rounded-full border-gray-300 text-yellow-400">
                        </div>
                      </div>
