...
<div class="px-4 sm:px-6 lg:px-8 mt-8">
    ...
    <form method="post" action="{{ route('servicecategories.settranslation', ['category' => $category['id'], 'lang' => $lang]) }}" class="space-y-8 divide-y divide-gray-200" enctype="multipart/form-data">
        @method('PUT')  
        @csrf  
        ...
        <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
          <dl class="sm:divide-y sm:divide-gray-200">
                ...
                <div class="mt-1 sm:col-span-2 sm:mt-0">
                  <select id="department_id" name="department_id" class="block w-full max-w-lg rounded-md border-gray-300 shadow-sm sm:max-w-xs sm:text-sm">
                    @foreach ($departments as $department)
                        <option value="{{ $department['id'] }}" @selected($department['department_id'] == $department['id'])>{{ $department['name'] }}</option>
                    @endforeach
                  </select>
                </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $category['email'] }}" type="email" name="email" id="email" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('email')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('email'))
