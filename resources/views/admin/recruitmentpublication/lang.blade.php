...
<div class="px-4 sm:px-6 lg:px-8 mt-8">
    ...
    <form method="post" action="{{ route('recruitmentpublications.settranslation', ['publication' => $publication['id'], 'lang' => $lang]) }}" class="space-y-8 divide-y divide-gray-200" enctype="multipart/form-data" x-data="imgPreview" x-cloak>
        @method('PUT')  
        @csrf  
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
          <dl class="sm:divide-y sm:divide-gray-200">
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $publication['start_date'] }}"
                           type="datetime-local"
          ...
          <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            ...
            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
              <div class="mt-1">
                  <input value="{{ $publication['end_date'] }}"
                        type="datetime-local"
                        pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}"
                        name="end_date" 
                        id="end_date" 
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Horário: Início</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $publication['start_time'] }}"
                           type="time"
                           step="2"
                           name="start_time" 
                           id="start_time" 
          ...
          <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <dt class="text-sm font-medium text-gray-500">Horário: Fim</dt>
            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
              <div class="mt-1">
                  <input value="{{ $publication['end_time'] }}"
                        type="time"
                        step="2"
                        name="end_time" 
                        id="end_time" 
        ...
        <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
          ...
          <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
            <div class="mt-1">
              <textarea id="description" name="description" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('description')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">{{ $publication['description'] }}</textarea>
                </div>
        ...
        <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
          ...
          <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
            <div class="mt-1">
              <textarea id="requirements" name="requirements" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('requirements')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">{{ $publication['requirements'] }}</textarea>
                </div>
                @if($errors->has('requirements'))
                <p class="mt-2 text-sm text-red-500">{{ $errors->first('requirements') }}</p>
                @endif
          </dd>
        </div>
                ...
                <div class="divide-y divide-gray-200">
                  <div class="relative flex items-start py-4">
                    <div class="min-w-0 flex-1 text-sm">
                      ...
                      <p id="status-description" class="text-gray-500">Define se a publicação em questão continua activa.</p>
                    </div>
                    <div class="ml-3 flex h-5 items-center">
                      <input value="1" @checked($publication['status']) id="status" aria-describedby="status-description" name="status" type="checkbox" class="h-4 w-4 rounded-full border-gray-300 text-yellow-400">
                    </div>
                  </div>
                        ...
                        @forelse ($publication['documents'] as $document)
                          <li class="py-4">
                            <div class="flex items-center space-x-4">
                              ...
                              <div>
                                <a href="{{ route('recruitmentpublications.deletesingleattachment', ['model_id' => $document->id]) }}" class="ml-3 rounded-md inline-flex justify-center px-2.5 py-1.5 text-xs font-medium bg-gray-900 text-white shadow-sm hover:bg-white hover:text-red-500 sm:w-auto">
