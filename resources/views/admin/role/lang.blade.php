...
<div class="px-4 sm:px-6 lg:px-8 mt-8">
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        <h1 class="text-xl font-semibold text-gray-900">{{ $role['name'] }}</h1>
    ...
    <form method="post" action="{{ route('roles.settranslation', ['role' => $role['id'], 'lang' => $lang]) }}" class="space-y-8 divide-y divide-gray-200" enctype="multipart/form-data" x-data="{}" x-cloak>
        @method('PUT')  
        @csrf  
          ...
          <dl class="sm:divide-y sm:divide-gray-200">
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $role['name'] }}" type="text" name="name" id="name" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('name')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('name'))
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $role['label'] }}" type="text" name="label" id="label" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('label')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('label'))
                    ...
                    <div class="divide-y divide-gray-200" x-data="checkboxes()">
                      ...
                      @foreach ($permissions as $permission)
                        <div class="relative flex items-start py-4">
                          ...
                          <div class="ml-3 flex h-5 items-center">
                            <input @checked(in_array($permission['id'], $role['permissions'])) x-model="permissions" value="{{ $permission['id'] }}" id="permission-{{ $permission['id'] }}" aria-describedby="permission-{{ $permission['id'] }}-description" name="permissions[]" type="checkbox" class="h-4 w-4 selectAll rounded-full border-gray-300 text-yellow-400 focus:ring-yellow-400">
                          </div>
                        </div>
                      @endforeach
