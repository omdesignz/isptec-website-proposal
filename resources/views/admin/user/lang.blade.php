...
<div class="px-4 sm:px-6 lg:px-8 mt-8">
    ...
    <form method="post" action="{{ route('users.settranslation', ['user' => $user['id'], 'lang' => $lang]) }}" class="space-y-8 divide-y divide-gray-200" enctype="multipart/form-data" x-data="{}" x-cloak>
        @method('PUT')  
        @csrf  
          ...
          <dl class="sm:divide-y sm:divide-gray-200">
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $user['name'] }}" type="text" name="name" id="name" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('name')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('name'))
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $user['email'] }}" type="email" name="email" id="email" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('email')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('email'))
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $user['username'] }}" type="text" name="username" id="username" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('username')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Senha Actual</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ old('current_password') }}" type="password" name="current_password" id="current_password" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('current_password')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('current_password'))
                    <p class="mt-2 text-sm text-red-500">{{ $errors->first('current_password') }}</p>
                    @endif
              </dd>
            </div>

            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Nova Senha</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">Confirmar Nova Senha</dt>
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    ...
                    <div class="divide-y divide-gray-200" x-data="rolecheckboxes()">
                      ...
                      @foreach ($roles as $role)
                        <div class="relative flex items-start py-4">
                          ...
                          <div class="ml-3 flex h-5 items-center">
                            <input @checked(in_array($role['id'], $user['roles'])) x-model="roles" value="{{ $role['id'] }}" id="role-{{ $role['id'] }}" aria-describedby="role-{{ $role['id'] }}-description" name="roles[]" type="checkbox" class="h-4 w-4 roleselectAll rounded-full border-gray-300 text-yellow-400 focus:ring-yellow-400">
                          </div>
                        </div>
                      @endforeach
                    ...
                    <div class="divide-y divide-gray-200" x-data="checkboxes()">
                      ...
                      @foreach ($permissions as $permission)
                        <div class="relative flex items-start py-4">
                          ...
                          <div class="ml-3 flex h-5 items-center">
                            <input @checked(in_array($permission['id'], $user['permissions'])) x-model="permissions" value="{{ $permission['id'] }}" id="permission-{{ $permission['id'] }}" aria-describedby="permission-{{ $permission['id'] }}-description" name="permissions[]" type="checkbox" class="h-4 w-4 selectAll rounded-full border-gray-300 text-yellow-400 focus:ring-yellow-400">
                          </div>
                        </div>
                      @endforeach
