...
<div class="px-4 sm:px-6 lg:px-8 mt-8">
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        <h1 class="text-xl font-semibold text-gray-900">{{ $footprint['name'] }}</h1>
    ...
    <form method="post" action="{{ route('academicfootprints.settranslation', ['footprint' => $footprint['id'], 'lang' => $lang]) }}" class="space-y-8 divide-y divide-gray-200" enctype="multipart/form-data" x-cloak>
        @method('PUT')  
        @csrf  
        ...
        <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
          <dl class="sm:divide-y sm:divide-gray-200">
                ...
                <div class="mt-1 sm:col-span-2 sm:mt-0">
                  <select id="category_id" name="category_id" class="block w-full max-w-lg rounded-md border-gray-300 shadow-sm sm:max-w-xs sm:text-sm">
                    @foreach ($categories as $category)
                        <option value="{{ $category['id'] }}" @selected($footprint['category_id'] == $category['id'])>{{ $category['name'] }}</option>
                    @endforeach
                  </select>
                </div>
                ...
                <div class="mt-1 sm:col-span-2 sm:mt-0">
                  <select id="employee_id" name="employee_id" class="block w-full max-w-lg rounded-md border-gray-300 shadow-sm sm:max-w-xs sm:text-sm">
                    @foreach ($employees as $employee)
                        <option value="{{ $employee['id'] }}" @selected($footprint['employee_id'] == $employee['id'])>{{ $employee['full_name'] }}</option>
                    @endforeach
                  </select>
                </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $footprint['name'] }}" type="text" name="name" id="name" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('name')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('name'))
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $footprint['link'] }}" type="url" name="link" id="link" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('link')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('link'))
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                  <textarea id="description" name="description" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('description')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">{{ $footprint['description'] }}</textarea>
                    </div>
