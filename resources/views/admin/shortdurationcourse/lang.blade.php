...
<div class="px-4 sm:px-6 lg:px-8 mt-8">
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        <h1 class="text-xl font-semibold text-gray-900">{{ $course['name'] }}</h1>
    ...
    <form method="post" action="{{ route('shortdurationcourses.settranslation', ['course' => $course['id'], 'lang' => $lang]) }}" class="space-y-8 divide-y divide-gray-200" enctype="multipart/form-data" x-data="imgPreview" x-cloak>
        @method('PUT')  
        @csrf  
        ...
        <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
          <dl class="sm:divide-y sm:divide-gray-200">
                ...
                <div class="mt-1 sm:col-span-2 sm:mt-0">
                  <select id="department_id" name="department_id" class="block w-full max-w-lg rounded-md border-gray-300 shadow-sm sm:max-w-xs sm:text-sm">
                    @foreach ($departments as $department)
                        <option value="{{ $department['id'] }}" @selected($course['department_id'] == $department['id'])>{{ $department['name'] }}</option>
                    @endforeach
                  </select>
                </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $course['name'] }}" type="text" name="name" id="name" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('name')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
                    @if($errors->has('name'))
                ...
                <div class="mt-1 sm:col-span-2 sm:mt-0">
                  <select id="employee_id" name="employee_id" class="block w-full max-w-lg rounded-md border-gray-300 shadow-sm sm:max-w-xs sm:text-sm">
                    @foreach ($employees as $employee)
                        <option value="{{ $employee['id'] }}" @selected($course['employee_id'] == $employee['id'])>{{ $employee['full_name'] }}</option>
                    @endforeach
                  </select>
                </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                    <input value="{{ $course['external_employee'] }}" type="text" name="external_employee" id="external_employee" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('external_employee')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">
                    </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <div class="mt-1">
                  <textarea id="description" name="description" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm @if($errors->has('description')) focus:border-rose-500 focus:ring-rose-500 @endif sm:text-sm">{{ $course['description'] }}</textarea>
                    </div>
                    ...
                    <div class="divide-y divide-gray-200">
                      <div class="relative flex items-start py-4">
                        ...
                        <div class="ml-3 flex h-5 items-center">
                          <input value="1" @checked($course['status']) id="status" aria-describedby="status-description" name="status" type="checkbox" class="h-4 w-4 rounded-full border-gray-300 text-yellow-400">
                        </div>
                      </div>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                @if (count($course['featured_image']) > 0)
                  <img src="{{ $course['featured_image'][0]?->getUrl() }}" alt="" class="object-cover h-96 w-96 rounded-2xl" loading="lazy" />
                @endif
                        ...
                        @forelse ($course['documents'] as $document)
                          <li class="py-4">
                            <div class="flex items-center space-x-4">
                              ...
                              <div>
                                <a href="{{ route('shortdurationcourses.deletesingleattachment', ['model_id' => $document->id]) }}" class="ml-3 rounded-md inline-flex justify-center px-2.5 py-1.5 text-xs font-medium bg-gray-900 text-white shadow-sm hover:bg-white hover:text-red-500 sm:w-auto">
    ...
    document.addEventListener('alpine:init', () => {
        ...
        Alpine.data('multipleFileViewer', () => ({
            ...
            previewFiles() {
              ...
              picReader.addEventListener('load', (course) => {
